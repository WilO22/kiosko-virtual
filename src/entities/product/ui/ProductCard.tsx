...
export function ProductCard({ product, onAddToCart }: ProductCardProps) {
  ...
  return (
    <div className="group flex flex-col bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 hover:border-brand-orange/30 transition-all duration-300 overflow-hidden h-full">
      {/* Imagen (clickeable) */}
      <Link to={`/producto/${id}`} className="relative h-48 w-full overflow-hidden bg-gray-50 flex-shrink-0 block">
        <img
          src={imageUrl}
          alt={name}
      ...
      <div className="flex flex-col flex-grow p-5 justify-between">
        <div>
          <Link to={`/producto/${id}`} className="hover:text-brand-orange transition-colors">
